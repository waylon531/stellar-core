                                   ...
                                   ValueWrapperPtrSet const& candidates)
{
    ...
    using HighestPair = std::pair<StellarValue, TxSetFramePtr>;
    HighestPair highestPair = std::accumulate(
        candidateValues.begin(), candidateValues.end(), HighestPair(),
        [&](HighestPair const& hp, StellarValue const& sv) {
            auto const& highestTxSet = hp.second;
            auto const& svSetHash = sv.txSetHash;
            auto cTxSet = mPendingEnvelopes.getTxSet(svSetHash);
            return (cTxSet && cTxSet->previousLedgerHash() == lcl.hash &&
                    (!highestTxSet ||
                     compareTxSets(highestTxSet, cTxSet, hp.first.txSetHash,
                                   svSetHash, lcl.header, candidatesHash)))
                       ? HighestPair(sv, cTxSet)
                       : hp;
        });

    comp = highestPair.first;
